...
static size_t mystrlen(const char * n_)
{
  size_t i=0;
  while ( n_[i]!='\0') ++i;
  return i;
...
int GmfOpenMesh(const char *FilNam, int mod, ...)
{
...
#if 0
  printf("OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO %s\n",FilNam);
  printf("OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO %d\n",strlen(FilNam) + 7);
#endif
  if(mystrlen(FilNam) + 7 >= GmfStrSiz)
    return(0);
  ...
  if(msh->mod == GmfRead)
      ...
      if(!(msh->hdl = fopen(msh->FilNam, "rb")))
	{
      ...
      if(msh->typ & Bin)
	{
	  if (!fread((unsigned char *)&msh->cod, WrdSiz, 1, msh->hdl))
	    ((void)0);
      ...
      else
	{
	  ...
	  /* ON FORCE LA VERSION DOUBLE */

	  msh->ver=2;
	  if (!fscanf(msh->hdl, "%d", &msh->dim))
	    ((void)0);
...
int GmfSetKwd(int MshIdx, int KwdCod, ...)
{
  ...
  if(mystrlen(GmfKwdFmt[ KwdCod ][2]))
    {
      va_start(VarArg, KwdCod);
  ...
  if(kwd->typ != SolKwd)
      ...
      if(msh->ver == 1)
	{
	  ...
	  if(msh->typ & Asc)
	    for(j=0;j<kwd->SolSiz;j++)
	      {
		if (!fscanf(msh->hdl, "%f", &FltSolTab[j]))
		  ((void)0);
      ...
      else if(msh->ver == 2)
	{
	  ...
	  if(msh->typ & Asc)
	    for(j=0;j<kwd->SolSiz;j++)
	      {
		if (!fscanf(msh->hdl, "%lf", &DblSolTab[j]))
	    ((void)0);
  ...
  for(i=0;i<kwd->SolSiz;i++)
      ...
      if(kwd->fmt[i] == 'r')
	{
	  if(InpMsh->ver == 1)
	    {
	      if(InpMsh->typ & Asc)
		{if (!fscanf(InpMsh->hdl, "%f", &f)) ((void)0);}
	      else
	    ...
	    {
	      if(InpMsh->typ & Asc)
		{ if (!fscanf(InpMsh->hdl, "%lf", &d) )((void)0);}
	      else
  ...
  if(!strcmp("i", GmfKwdFmt[ KwdCod ][2]))
    {
      if(msh->typ & Asc)
	{
	  if (!fscanf(msh->hdl, "%d", &kwd->NmbLin))
	    ((void)0);
  ...
  if(!strcmp("sr", GmfKwdFmt[ KwdCod ][3]))
    {
      if(msh->typ & Asc)
	{
	  if (!fscanf(msh->hdl, "%d", &kwd->NmbTyp))
	    ((void)0);


	  for(i=0;i<kwd->NmbTyp;i++)
	    {
	      if (!fscanf(msh->hdl, "%d", &kwd->TypTab[i]))
	    ((void)0);
...
static void ExpFmt(GmfMshSct *msh, int KwdCod)
{
  ...
  size_t i = kwd->SolSiz = 0;

  while(i < mystrlen(InpFmt))
    {
      chr = InpFmt[ i++ ];
...
static void ScaWrd(GmfMshSct *msh, unsigned char *wrd)
{
  ...
  if (1!=fread(wrd, WrdSiz, 1, msh->hdl))
    ((void)0);
...
static void ScaDblWrd(GmfMshSct *msh, unsigned char *wrd)
{
  ...
  if (1!=fread(wrd, WrdSiz, 2, msh->hdl))
    ((void)0);
...
static void ScaBlk(GmfMshSct *msh, unsigned char *blk, int siz)
{
  ...
  if (1!=fread(blk, WrdSiz, siz, msh->hdl))
    ((void)0);
...
int call(gmfsetkwdf77)(int *MshIdx, int *KwdIdx, int *NmbLin, int *NmbTyp, int *TypTab)
{
  ...
  else if(mystrlen(GmfKwdFmt[ *KwdIdx ][2]))
